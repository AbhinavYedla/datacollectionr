...
nationalities <- read.delim("Nationalities.txt", sep="\t")
total.nationalities <- nrow(nationalities)
...
for (i in 217:total.nationalities) {
  query <- as.character("Why are ")
  target.region <- nationalities[i, 2]
